...
@interface HDZLifeViewController ()<UIScrollViewDelegate>
{
...
@implementation HDZLifeViewController
...
- (void)setupUnderLineView
{
    ...
    // 下滑线高度
    CGFloat lineViewH = 2;
    CGFloat y = _view.hdz_height - lineViewH;
    lineView.hdz_height = lineViewH;
    lineView.hdz_y = y;
    // 设置下划线的宽度比文本内容宽度大10
    [titleBtn.titleLabel sizeToFit];
    lineView.hdz_width = titleBtn.titleLabel.hdz_width + 2;
    lineView.hdz_centerX = titleBtn.hdz_centerX;
    // 添加到titleView里
...
- (void)titleBtnClick:(UIButton *)titleBtn{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.lineView.hdz_width = titleBtn.titleLabel.hdz_width;
        self.lineView.hdz_centerX = titleBtn.hdz_centerX;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    // 计算拖拽的比例
    CGFloat ratio = scrollView.contentOffset.x / scrollView.hdz_width;
    ...
    if (isClick) {
        ...
        self.lineView.hdz_x = titleBtn.titleLabel.hdz_x;
        self.lineView.hdz_width = titleBtn.titleLabel.hdz_width;
        self.lineView.hdz_centerX = titleBtn.hdz_centerX;
        isClick = YES;
    }else{
...
//        }
        ...
        if (ratio > 0) {
            self.lineView.hdz_x = self.preBtn.titleLabel.hdz_x;
            self.lineView.hdz_width = self.preBtn.hdz_centerX + scrollView.contentOffset.x / 2.5 + 15;
            if (scrollView.contentOffset.x > 180) {
                UIButton * btn = self.titleBtns[1];
                self.lineView.hdz_x =  scrollView.contentOffset.x / 2.5 + 15 - self.preBtn.hdz_centerX;
                self.lineView.hdz_width = (btn.hdz_centerX + btn.hdz_width) - (scrollView.contentOffset.x / 2.5) - 45;
            }
        }else{
            self.lineView.hdz_x = 15 + scrollView.contentOffset.x / 5 ;
            self.lineView.hdz_width = self.preBtn.hdz_centerX - (scrollView.contentOffset.x / 5);
            if (scrollView.contentOffset.x < 180) {
                UIButton * btn = self.titleBtns[0];
                self.lineView.hdz_x = btn.titleLabel.hdz_x;
                self.lineView.hdz_width  = btn.hdz_width + (scrollView.contentOffset.x / 5) - 20;
            }
        }
